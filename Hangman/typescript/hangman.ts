...
var guessesRemaining: number = 8;
var words: string[] = ["javascript", "java", "python", "typescript", "hangman", "programmer",
    ...
    if (/^[a-z]{1}$/i.test(data)) {
        if (playedLetters.indexOf(data) >= 0) {
            ...
            if (word.indexOf(data) >= 0) {
                for (var i: number = 1; i < word.length - 1; i++) {
                    if (word[i] === data) {
                        obfWord[i] = word[i];
                        wordl--;
                    }
...
function clr(text: string, color: string): string {
    switch (color) {
        case "red":
